...
def find_all_of_letter (letter):
  pos_list = []
  for i in range(len(f)):
    for j in range(len(f[i])):
      if f[i][j] == letter or (letter == "S" and f[i][j] == "a"):
        pos_list.append((i, j))
  return pos_list
...
def find_path_back(node, path_length=0):
  if node.parent:
    return find_path_back(node.parent, path_length+1)
  else:
    return path_length
...
def a_star(start_pos):
  ...
  while open_list:
    ...
    if current_node == end_node:
      return find_path_back(current_node)
...
result_list = []
positions_to_check = find_all_of_letter("a")
print("Finding shortest path from", len(positions_to_check), "starting positions")
for i in range(len(positions_to_check)):
  path_found = a_star(positions_to_check[i])
  print("  --- " + str(int(i / len(positions_to_check) * 100)) + "% complete ---  ", end="\r")
  if path_found != None:
    result_list.append(path_found)
print("  === Result found:",  min(result_list), "===  ")
